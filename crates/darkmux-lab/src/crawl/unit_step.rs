            ...
            if let Some(compare) = &rule.compare {
                out.push_str(&format!(
                    "\nBefore you call create_finding for this pattern: answer this question in one line, then \
                     ...
                     QUESTION for the author, not a claim, so be honest that it is unconfirmed: {compare} Set the \
                     `answer` key to your one-word answer (`yes`, `no`, `partly`, or `cannot_tell`). If your answer \
                     is `no` or `cannot_tell`, do NOT call create_finding for this pattern at all — a negative or \
                     unconfirmable answer is not a finding, and every conversation you raise costs the author a \
                     click.\n",
                ));
            }
        }

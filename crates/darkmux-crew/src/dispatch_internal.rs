...
///
/// - `Some(dir)`: the caller named an exact dir. A caller-named path's
///   parent is expected to already exist (e.g. the crawl launcher's own
///   `runs/<mission>/units/` dir).
/// - `None`: today's behavior — a fresh tempdir keyed off `role_id` and
///   this dispatch's own unix-micros timestamp. This name is predictable
///   ahead of creation, which is exactly the property #2158 exploited —
///   `create_dir_exclusive_unique_0700` (not this function's job to
///   re-derive) is what makes a pre-planted symlink or leftover dir at that
///   name a STEP TO THE NEXT NAME instead of a silent redirect of the
...
fn resolve_host_out(host_out_override: Option<&Path>, role_id: &str, unix_micros: u128) -> Result<PathBuf> {
    match host_out_override {
        Some(dir) => {
            create_dir_exclusive_0700(dir)?;
            Ok(dir.to_path_buf())
        }
        None => {
            let base = std::env::temp_dir().join(format!("darkmux-out-{role_id}-{unix_micros}"));
            // (#2158) Auto-named: a collision means "pick the next name",
            // never "reuse what is there" — see
            // `create_dir_exclusive_unique_0700`'s own doc for why a hard
            // refusal here would be an availability cliff.
            create_dir_exclusive_unique_0700(&base)
                .with_context(|| format!("creating dispatch out-dir: {}", base.display()))
        }
    }
}
...
/// panic until `n` reaches approximately `9.22e18` — not the 315_360_000
/// (~10 years) the clamp used. That figure is NOT a fixed constant: it is
/// `Instant::now()`-relative, so it walks down as the process's own uptime
/// advances — independently re-running the same binary search (#2641
/// follow-up review, CONSIDER 4) reproduced the ~9.22e18 order of
/// magnitude but landed on a different exact value than an earlier
/// measurement of the same search, off by roughly two thousand. See the
/// tracking issue filed alongside this revert
/// (#2639) for the conditions a real clamp needs before it returns: routed
/// through THIS function so the bounds record can't desync, self-
/// disclosing (a resolution-time warning + a `darkmux doctor` row), and
/// sized from a measured, uptime-dependent ~9.22e18 ceiling — not a single
/// number a future implementer can hardcode.
...
/// rewritten for the container's view of the network: a loopback host
/// names the container itself inside Docker, so it becomes
/// `host.docker.internal` (Docker Desktop's name for the host, which the
/// runtime's baked-in default already relies on). Any other host, the port,
/// and the path pass through unchanged. This is a networking translation
/// only; it says nothing about where the endpoint is.
///
...
pub(crate) fn container_lmstudio_base_url(override_url: Option<&str>) -> Option<String> {
    ...
    let base = crate::single_shot::lmstudio_v1_base(&darkmux_types::config_access::lmstudio_url());
    Some(loopback_to_docker_host(&base))
}

/// Rewrite a loopback host (`localhost`, `127.0.0.1`, `[::1]`, any case) in
/// `url` to `host.docker.internal`, keeping scheme, port, and path. Any
...
fn loopback_to_docker_host(url: &str) -> String {
    ...
    // `[::1]:port` keeps its colon inside the brackets; split the port off
    ...
    format!("{scheme}://host.docker.internal{port}{path}")
}
...
/// `stop_flag`, the telemetry sampler's `sampler_stop`, and (#2641
/// follow-up review, MUST FIX 2) the inactivity watchdog's
/// `watchdog_abandoned` — deliberately NOT `watchdog_done` (see
/// `run_watchdog`'s own doc for why guarding `watchdog_done` itself was
/// wrong: it is the ONLY signal that licenses the watchdog to skip its
/// kill entirely, and a panicked main thread has proven nothing about
/// whether the container is actually gone). All three follow the
/// identical broken shape this guard exists to close: every EXPLICIT
/// `.store(true, …)` call on each flag sits on a normal-return path in
/// `dispatch()` (the `wait_with_output` error arm, the
/// interrupt-and-non-success branch, the natural end-of-function success
/// path — see each thread's own spawn site for its specific set), so a
/// PANIC unwinding through the lines between a flag's creation and those
...
/// unguarded: the tailer keeps polling `trajectory.jsonl` every 250ms
/// forever; the sampler keeps shelling out to enumerate loaded models,
/// sampling host telemetry, and running the thermal governor every 2s
/// forever — against a dead dispatch's session/role/mission, and able to
/// write `pace.json` or a crawl mission's STOP file for a run that no
/// longer exists; and (pre-#2641) the watchdog kept polling to its FULL
/// inactivity deadline — which can be minutes — before firing the
/// persistent kill, leaving `ContainerKillGuard` (#2233)'s single
/// fire-and-forget `docker kill` as the only thing actually trying to
/// stop the container for the whole gap. `watchdog_abandoned` closes that
/// gap: the watchdog's poll loop checks it every 500ms and, once set,
/// breaks out of the deadline wait immediately and runs the SAME
/// persistent, retrying, probe-confirmed kill (`watchdog_finalize_kill`)
/// a genuine timeout would — no watered-down substitute, just sooner.
///
...
///   has PROVEN the wait is over (`wait_with_output()` returned, an error
///   arm ran, an interrupt was handled). Seeing it true means "there is
///   nothing left to kill, or something else already handled it" — the
///   watchdog returns WITHOUT running `watchdog_finalize_kill` at all.
/// - `watchdog_abandoned` — set by `spawn_guarded_watchdog`'s
///   `StopFlagGuard`, whose Drop fires on ANY exit from the scope that
///   holds it, panic included. Seeing it true proves nothing about the
///   container's state (a panicked main thread never got the chance to
///   prove anything) — it means "the thing that was supposed to prove it is
///   gone, stop waiting on the deadline and go find out now" via the SAME
///   persistent, retrying, probe-confirmed kill a genuine timeout runs.
///
/// This is why `watchdog_done` itself must NEVER be the flag a panic-only
/// guard sets (the #2234-follow-up shape this replaces did exactly that):
/// `watchdog_done = true` on an unproven panic path would make the loop
/// return at the top with NO kill at all — worse than the pre-#2234
/// baseline, which at least eventually timed out and killed. Guarding
/// `watchdog_abandoned` instead can only ever ADD urgency (skip the
/// deadline wait, kill sooner) and never SUPPRESS the one kill this thread
/// exists to guarantee.
fn run_watchdog(
...
/// `StopFlagGuard` AND spawn the real `run_watchdog` thread together, as ONE
/// function — see `spawn_guarded_tailer`'s own doc for why this shape
/// matters: a test that calls THIS exact function (not a hand-rolled
/// `StopFlagGuard` stand-in) proves `dispatch()`'s own wiring, that the
/// guard returned here really wraps the SAME `watchdog_abandoned` flag the
/// really-spawned thread reads. `dispatch()` calls this one function to get
/// both; there is no second, guard-less way in this file to obtain a
/// watchdog thread. See
...
pub fn dispatch(opts: DispatchOpts) -> Result<DispatchResult> {
    ...
    // (#2641 follow-up review, MUST FIX 2) Deliberately a SEPARATE flag from
    // `watchdog_done` — see `run_watchdog`'s own doc for the full trade this
    // resolves. In one sentence: `watchdog_done` means "the main thread
    // proved the wait is over, skip the kill", which a panic never proves,
    // so guarding it (the #2234-follow-up shape this replaces) silently
    // traded away the only persistent, retrying, probe-confirmed kill on
    // the unwind path for `ContainerKillGuard`'s single fire-and-forget
    ...
    // (#2641 follow-up review, MUST FIX 1) Construct the watchdog's
    // panic-safe `StopFlagGuard` AND spawn the real `run_watchdog` thread
    // together, as ONE function — same shape as `spawn_guarded_tailer`
    // (see that function's own doc for why: a test calling THIS exact
    // function proves `dispatch()`'s own wiring, not just Drop semantics on
    // a hand-rolled guard). See
    // `spawn_guarded_watchdog_wiring_survives_a_real_thread_spawn` for the
    // red-prove.
    let (_watchdog_abandon_guard, watchdog_handle) = spawn_guarded_watchdog(
        &watchdog_abandoned,
    ...
    // (#2641 follow-up review, MUST FIX 1) Construct the sampler's
    // panic-safe `StopFlagGuard` AND spawn the real `run_telemetry_sampler`
    // thread together, as ONE function — same shape as `spawn_guarded_tailer`
    // (see that function's own doc for why: a test calling THIS exact
    // function proves `dispatch()`'s own wiring, not just Drop semantics on
    // a hand-rolled guard). This one is strictly worse than the tailer's
    // original gap: left unguarded on a panic, the sampler keeps shelling
    // out to enumerate loaded models + running the host probe, emitting
    // `category=telemetry` flow records stamped with the DEAD dispatch's
    // session/role/mission, and running the thermal governor (which can
    // write `pace.json` or a crawl mission's STOP file) every
    // `TELEMETRY_SAMPLE_INTERVAL` (2s) forever, in any process that
    // survives the panic — polluting the flow stream for a session the
    // viewer already shows as errored, and able to halt an unrelated live
    // crawl. See `spawn_guarded_sampler_wiring_survives_a_real_thread_spawn`
    // for the red-prove.
...
/// milliseconds, not a `Duration`, and the sampler-loop tests in
/// `dispatch_internal_tests.rs` (that file is `include!`'d as this
/// module's own `mod tests` via `super::*`, so it reads this constant
/// without `pub` at all). (#2413 C7, corrected round 3 MF2) The doctor
/// check this constant's `pub` visibility used to serve, `check_
/// telemetry_record_every_samples`, is retired along with the config knob
/// it surfaced, and — checked directly — nothing outside this file reads
/// this constant either. `pub` was never actually load-bearing for
/// anything beyond this module; dropped.
const TELEMETRY_SAMPLE_INTERVAL_MS: u64 = 2000;

/// (#2413 M2, review round 2) Whether a machine-scoped `machine.telemetry`
/// emission is due on this tick. The dispatch-owned sampler's outer loop
/// ticks every `TELEMETRY_SAMPLE_INTERVAL_MS` (2s) for the things that
/// stay on that cadence (lms diffs, the thermal governor); the
/// `machine.telemetry` curve itself must honor the operator's configured
/// `host_sampler_interval_ms()` knob instead of piggybacking on the 2s
/// tick, or it silently emits ~2.5x more records than the `interval_ms`
/// it stamps into each one claims. Pure — testable without a real
/// sampler thread. `last_emit_at_ms` is `None` before the first
/// emission (always due).
pub(crate) fn telemetry_emission_due(last_emit_at_ms: Option<u64>, now_at_ms: u64, interval_ms: u64) -> bool {
    match last_emit_at_ms {
        None => true,
        Some(last) => now_at_ms.saturating_sub(last) >= interval_ms,
    }
}

/// (#2413 round 3 MF2) Decide whether THIS tick emits a `machine.telemetry`
/// record, and if so, build it — extracted out of the sampler loop's
/// `if telemetry_emission_due(...) { ... }` block so that CALL SITE is
/// directly testable with a fake clock, not just its dependency
/// (`telemetry_emission_due`) in isolation. A prior review round's
/// mutation of the real loop's `if` (swapped for `if true`) stayed green
/// against the existing suite — nothing exercised the wiring, only the
/// pure gate function. This function's own test drives it across a
/// sequence of `at_ms` ticks and counts emissions; swapping ITS internal
/// `if telemetry_emission_due(...)` for `if true` must turn that count
/// wrong.
///
/// Stamps the MEASURED gap since the last emission as `interval_ms`
/// (`at_ms - last_emit_at_ms`), not the configured knob verbatim — a tick
/// that runs late (a slow probe, a paused thread) reports what actually
/// happened. The very first emission (`last_emit_at_ms: None`) has no
/// prior gap to measure, so it stamps the knob itself.
fn maybe_build_machine_telemetry_record(
    last_emit_at_ms: Option<u64>,
    at_ms: u64,
    sampled_at_epoch_ms: u64,
    interval_ms: u64,
    sample: &crate::host_probe::HostSampleFull,
) -> Option<(darkmux_flow::FlowRecord, u64)> {
    if !telemetry_emission_due(last_emit_at_ms, at_ms, interval_ms) {
        return None;
    }
    let effective_interval_ms = match last_emit_at_ms {
        Some(last) => at_ms.saturating_sub(last),
        None => interval_ms,
    };
    let rec = crate::host_probe::build_machine_scoped_telemetry_record(sample, sampled_at_epoch_ms, effective_interval_ms);
    Some((rec, at_ms))
}
...
/// (#2779) The `thermal.stop_unresolved` payload — the breaker could not
/// write (or could not resolve) the crawl STOP file, reported with the
/// thermal `state` that tripped it.
///
/// Extracted the way `runtime_rest_payload` was, and for the same reason:
/// the two emission sites live on the sampler thread of a live dispatch,
/// which no in-process test reaches, so an inline `json!` was pinned by
/// nothing. `state` IS the host reading, which is what puts this record in
/// [`crate::host_source::HOST_READING_ACTIONS`] as `Stamped`.
fn thermal_stop_unresolved_payload(
    cause: &str,
    reason: &str,
    state: &str,
    provenance: &crate::host_source::Provenance,
...
/// (#2779) The `battery.pause_unsupported` payload — the operator asked for
/// a battery-floor pause and this run type cannot take one, so it continues
/// and says so loudly.
///
/// Extracted for the same reason as [`thermal_stop_unresolved_payload`].
/// `charge_pct` is the host reading; the record is `Level::Warn`, so a
/// fabricated low battery must not read as a real one here.
fn battery_pause_unsupported_payload(
    charge_pct: u8,
    floor_pct: u8,
    provenance: &crate::host_source::Provenance,
) -> serde_json::Value {
...
/// (#2779) Wrap an emitter so every record it carries is stamped.
///
/// Extracted from [`tier5_eject_on_critical`]'s body so the stamping is
/// pinned by BEHAVIOR rather than by the presence of a call: a test drives
/// this with a collecting closure and reads the payloads back. The eject
...
fn tier5_eject_on_critical(host_out: &Path, trip_wall: SystemTime, emit: &dyn Fn(&str, serde_json::Value)) {
    ...
    let emit = stamping_emitter(emit, crate::host_source::provenance());
...
/// (#2641 follow-up review, MUST FIX 1) Construct the sampler's panic-safe
/// `StopFlagGuard` AND spawn the real `run_telemetry_sampler` thread
/// together, as ONE function — same shape and same reasoning as
/// `spawn_guarded_tailer` (see that function's own doc): a test calling
/// THIS exact function proves `dispatch()`'s own wiring rather than just
/// Drop semantics on a hand-rolled guard. See
/// `spawn_guarded_sampler_wiring_survives_a_real_thread_spawn` for the
/// red-prove.
#[allow(clippy::too_many_arguments)]
...
fn run_telemetry_sampler(
    ...
    // (#2413 M2, review round 2) `at_ms` of the last `machine.telemetry`
    // EMISSION (not heartbeat) — feeds `telemetry_emission_due` so the
    // curve honors `machine_sampler_interval_ms` instead of the 2s outer
    ...
    loop {
            ...
            match event {
                ...
                crate::thermal_governor::ThermalEvent::Breaker { state } => {
                    ...
                    if let Some((cause, reason)) = unresolved {
                        ...
                        // `emit_rest` above.
                        let mut payload = thermal_stop_unresolved_payload(
                            cause.as_str(),
                            reason,
                            &state,
        ...
        if let Some(guard) = sampler_lock.as_ref() {
            if guard.heartbeat(machine_sampler_interval_ms) {
                // (#2413 M2 / round 3 MF2) The outer loop ticks every
                // `TELEMETRY_SAMPLE_INTERVAL_MS` (2s) — right for the lms
                // diffs and thermal governor above, which want fine-grained
                // sampling, but wrong for the `machine.telemetry` curve,
                // which must honor the operator's `host_sampler_interval_ms`
                // knob (`machine_sampler_interval_ms` here) or it silently
                // emits ~2.5x more records than its own `interval_ms` field
                // claims. The heartbeat above still runs every tick (cheap,
                // keeps the lock's staleness window nowhere near tripping);
                // only the flow-record emission itself is gated — decided
                // and built by `maybe_build_machine_telemetry_record`
                // (extracted so this call site is directly testable with a
                // fake clock; see that function's own test).
                //
                ...
                if let Some((mut rec, new_last_emit_at_ms)) = maybe_build_machine_telemetry_record(
                    last_telemetry_emit_at_ms,
                    at_ms,
                    sampled_at_epoch_ms,
                    ...
                    last_telemetry_emit_at_ms = Some(new_last_emit_at_ms);
                }
            } else {
                sampler_lock = None;
            }
        } else if machine_sampler_interval_ms > 0 {
            // (#2413 C1) Retry every tick even after failing (or never
            // holding) the lock — a file read every `TELEMETRY_SAMPLE_
            // INTERVAL_MS` (2s) is cheap, and it means a live dispatch
            // takes over the machine's sampler role as soon as whoever
            // held it (typically the daemon) dies mid-run, instead of the
            // machine going emitter-less until this dispatch happens to
            // restart. `0` (disabled) never retries, matching the initial
            // acquire's own `> 0` gate above.
            // (#2413 C2, and round 3 MF1) This is the SAME "opportunistic,
            // usually-declined" shape as the daemon's own per-tick retry
            // (see `darkmux-serve::host_sampler`'s identical comment) —
            // `try_acquire` never records a routine decline as contention
            // (the channel is retired; see `host_sampler_lock`'s module
            // doc), so this call is exactly as quiet as the initial
            // acquire above.
            sampler_lock = crate::host_sampler_lock::try_acquire("dispatch", machine_sampler_interval_ms);
            if sampler_lock.is_some() {
                // Fresh acquisition: the next tick's heartbeat branch
                // above will find `last_telemetry_emit_at_ms` still
                // reflecting whatever it was before (`None` at dispatch
                // start, or a stale past value if we previously held and
                // lost the lock) — either way `telemetry_emission_due`
                // treats a `None` as always-due, and a stale `Some` from
                // long before `at_ms` also reads as due, so the newly
                // reacquired sampler emits promptly rather than waiting a
                // full stale interval.
                last_telemetry_emit_at_ms = None;
            }
        }
...
fn probe_loaded_model_list() -> Result<Vec<String>> {
    // (#1863, named+tested #2534) This spawn bypasses darkmux-profiles'
    // `run_bounded` chokepoint entirely — it shells out to `lms` directly on
    // the dispatch path — so it needs its own cwd pin. A daemon/dispatch
    // process started from a git worktree that later gets deleted (routine
    // in this project's own workflow) otherwise inherits a cwd that no

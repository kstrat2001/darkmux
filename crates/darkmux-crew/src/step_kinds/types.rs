...
pub trait StepKind: Send + Sync {
    ...
    /// (#1230 Packet 4) Which local model, if any, this step needs
    ...
    /// `Placement` from its own config.
    ///
    /// **Best-effort, fails open.** This is a SCHEDULING CLASSIFICATION
    /// hint, not the dispatch's own model resolution — the step's `run`
    /// method still does its own full resolution when it actually
    /// executes. If this can't cleanly resolve a placement it returns
    /// `None` rather than erroring — worst case the step is scheduled as
    /// `Remote` (today's behavior for every kind), never a hard failure
    /// purely from misclassification.

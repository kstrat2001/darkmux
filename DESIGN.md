...
1. **Profile multiplexer.** `darkmux swap <profile>` switches the loaded model + context window + compaction settings to a named profile. The original tool, and still the floor everything else stands on.

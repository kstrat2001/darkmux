...
- **Coordination substrate**: Redis Streams via `RedisSink` (opt-in via `DARKMUX_REDIS_URL`). Two stream classes:
  - `darkmux:work` — one global work queue (#590). Publishers `XADD`; runners `XREADGROUP` + `XACK` via a single shared consumer group. First-claimant-wins is the allocation algorithm; the first available runner claims any job.

...
| `grown_from` on a grown step's `config` | the grow seam | the viewer; `finding list` (alongside `config.rule`) | `{task, item, index}` |
...
Provenance is on the record, not in the operator's head: every grown step's `config` carries `grown_from: {task, item, index}`, the item's `rule` lands on `config.rule` through the template (which is how the viewer and `finding list` separate tracks), the run's `graph-report.json` gains a `grown` entry per growth event naming the template, the producer, the artifact path, the item count and the real task ids minted, and one `mission.grow` flow record carries the same facts live. `mission status` prints "grew N task(s) from `<from>`".

...
|---|---|---|
| `LocalModel(placement)` | needs this model resident locally | gestalt plans a wave for it; it holds a residency lease so a concurrent darkmux command cannot pass-1-evict its model as not-desired, whether that command is running in ANOTHER process or is a same-process sibling dispatch (#2663) — a live sibling's model at the SAME model key but an insufficient context is protected too (#2669): the placement Blocks (with a bounded hold-not-fail retry) instead of unloading it out from under the sibling |

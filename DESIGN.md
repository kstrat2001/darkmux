...
- **Coordination substrate**: Redis Streams via `RedisSink` (opt-in via `DARKMUX_REDIS_URL`). Two stream classes:
  - `darkmux:work` — one global work queue (#590). Publishers `XADD`; workers `XREADGROUP` + `XACK` via a single shared consumer group. First-claimant-wins is the allocation algorithm; the first available worker claims any job.

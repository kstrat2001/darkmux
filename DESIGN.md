...
**It is still labeled a spike in its own module docs, and that label is honest.** What works, works in Zed today; what is spike-grade is named in `src/acp.rs` rather than papered over. The most fragile piece *was* review-stage progress, recognized by pattern-matching substrings out of the review subprocess's stderr; that whole path went with the bespoke review route in #2310 P4d, and `/review` now runs as an ordinary `mission launch`. A structured progress channel every command can render is still the real fix, and it is a feature, not a spike's job.

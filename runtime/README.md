...
**Image requirement:** minimal. The agent's `bash` tool prefers `bash` but falls back to `sh` when bash isn't installed, so bare-alpine / busybox images (`rust:alpine`, `*:alpine`) work too — only a POSIX `sh` is required, which every Linux image has. (Truly shell-less distroless images aren't supported.) The cached binary at `~/.darkmux/runtime/darkmux-runtime` is version-stamped AND keyed on the source image's own id (`docker image inspect`, #2386 C4) — a `cargo install` of a new darkmux build and a rebuild of the default image (same tag, new content) both re-extract it automatically on the next `dispatch --image`. A manual `rm ~/.darkmux/runtime/darkmux-runtime` is no longer needed for either case; `darkmux doctor`'s "runtime binary cache" check names the mismatch when one exists.

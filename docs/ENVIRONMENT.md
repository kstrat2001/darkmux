...
| `DARKMUX_HOST_SAMPLER_INTERVAL_MS` | `5000` | (#2107, #1833) The serve daemon's continuous host sampler: every N ms it reads CPU / memory / GPU utilization (sysctl, vm_stat, `ioreg`; zero model work) into an in-memory ring of the last 10 minutes and serves `now` + window stats (mean / p95 / max) as `load` on `GET /machine/resources`, which the viewer's machine drawer and modal read while open. Writes NO flow records. `0` disables. The sampler stamps its own cost (`sampler_cost_ms_mean`); measured ~770 ms per sample on an M5 Max, dominated by the `ioreg` GPU probe, so tune the interval against that. Maps to `runtime.host_sampler_interval_ms`; `darkmux doctor` shows the resolved value and the measured cost. |
...
| `DARKMUX_HOST_SAMPLER_INTERVAL_MS` (#2107 — the daemon's continuous host sampler cadence; `0` disables) | `runtime.host_sampler_interval_ms` |

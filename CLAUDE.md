...
- **Name the model-on-test when characterizing local-AI behavior.** darkmux's bake-off methodology hires specific models for specific roles (most recent hire as of 2026-05-13: D = `qwen3.6-35b-a3b-turboquant-mlx` for routine coding; B = `qwen3-coder-next-mlx` as single-shot prose reserve; A = `qwen3.5-122b-a10b` as heavy-reasoning defensive reserve — see LAB_NOTEBOOK Beat 25 and Article 3's bake-off section). But what's actually loaded in LMStudio at any moment may differ — operators swap for reasons (debugging, A/B comparison, evaluating a new candidate, defensive escalation, or simply not having swapped back after a focused test). When you (the orchestrator) characterize behavior from a dispatch — *"the local layer's response was X"* — **know which model produced it**. `darkmux doctor` shows the active profile; `lms ps` shows the loaded models. If the loaded model differs from the most recent hire and the analysis is making generalizable claims about *the local layer*, name it explicitly. *"D produced X"* and *"A produced X"* are not interchangeable conclusions. The operator may confirm the swap was intentional, redirect to the hired model, or accept the data as-is — but they need the surface to decide. Silent misattribution (analyzing dispatch outputs as if from the hired model when they're actually from a reserve) inherits class-wide errors into every downstream claim. The architectural fixes are tracked as [#159](https://github.com/kstrat2001/darkmux/issues/159) (recommendation registry per hardware tier) and [#160](https://github.com/kstrat2001/darkmux/issues/160) (per-role `agent.model` pinning); this anti-pattern is the awareness layer until both ship. *Not restriction — operators have preferences and models evolve.* Just awareness, surfaced.

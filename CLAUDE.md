...
| `DARKMUX_INACTIVITY_TIMEOUT_SECONDS` | `600` | Per-dispatch inactivity timeout — kills the container if no proof-of-work signal lands within this many seconds. **Resets** on each successful tool call (read / bash / edit / write) and on each compaction event. A dispatch making any tool calls stays alive indefinitely; only total silence triggers the kill. Pathological tool patterns (cycle on same args, cascade of failures, edit-drift on same file, reasoning loops) are caught by their dedicated detectors — the deadline trusts activity; the detectors catch struggle. (#457 + #464; renamed from `DARKMUX_RUNTIME_DEADLINE_SECONDS`) |

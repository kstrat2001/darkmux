...
✅ **v1.8.0**, semver-stable. v1.8.0 closes the dispatch-to-PR loop's learning loop: detector firings now distill into **cautions** that surface in the next coder brief, a durable **lessons** store (`darkmux lessons`) carries operator-authored conventions into the brief alongside them (#994), and `darkmux mission debrief` adds a closing read on a finished mission (#1000). The live observability viewer was hardened to show observed state rather than asserted fiction — flow-derived liveness when Redis presence is down (#1007), per-attempt drill-down scoping (#1013), and a utility card + spec line that report real residency and RAM (#1008). `FLOW_SCHEMA` bumped to 1.14.0 (`Stage::Debrief`); upgrade mixed-version fleets together. v1.7.0 added the loop lab (`darkmux lab loop`, #986) and reviewer-correction-as-finding (#453); v1.6.0 persisted adjudication corrections into the next brief (#849); v1.5.0 added the verifier-fabrication gate (#799). The dispatch-to-PR loop, the missions lens + `mission status`, live observability, and the Homebrew pipeline are the stable surface.

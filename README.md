...
darkmux is two things: a **mission orchestrator** and a **lab**. You define a mission in config, launch it with `darkmux mission launch <config>`, and watch it run as a live task graph in the browser: a crew of local-AI roles works the phases, every dispatch is gated on your sign-off, and each run finalizes into a typed envelope. The lab is the empirical half: `darkmux lab run <workload>` measures how your own hardware runs a workload (wall clock, compaction events, verify outcome), so the config choices behind your missions rest on numbers, not guesses. Underneath both, darkmux keeps the right models resident at the right context under your RAM budget. Every seat runs local or cloud: your own models off the meter, or a hosted endpoint when a role needs frontier weights. The first thing worth doing after setup is launching a mission and watching the graph come alive.
...
✅ **v2.5.0, the honesty release: everything that was wrong and said nothing.** Nearly every fix here is one defect wearing different clothes. A dead run read `running` forever, and runs, missions and phases each decided "alive" differently; they now share one liveness decision (#1642/#1633/#1621/#1632). A review with nothing to review posted the same signal as a broken one, so the session waiting on it would re-run, and since the input never changes, that is an unbounded retry loop. A benign-empty run now says the bundler worked and re-running will not help, and reads `Clean` rather than `Degenerate`, while an **error**-empty run still reads `Degenerate` (#1605/#1654). A crew route label carried an endpoint credential toward public artifacts on two of its three construction paths (#1660). `runtime.feedback_injection` was settable, typed, documented, and read by nothing: no host accessor, and the host never forwarded it into the container, so both tiers were inert (#1548). All six fleet e2e binaries opened with a redis guard whose false arm printed and **returned** (a silent pass), and no workflow ever installed redis, so a daemon boot test that takes 63 seconds locally was completing in eight milliseconds on the merge gate; the whole fleet layer was guarded by nothing while loudly reporting that it was guarded. Making them run required compiling darkmux on **Linux**, which had never once happened because every workspace-compiling job was macOS. It did not compile (#1662). The suite also gained the ability to audit itself: mutation testing on every PR diff plus nightly across the workspace, line coverage, and browser fixtures generated from the wire types so a viewer test cannot drift from the struct it renders (#1635/#1637). New: `Task.reads`, a run-scoped output ledger letting a task read any completed task's output without an edge being drawn between them, so cross-phase data flow stops demanding cross-phase arrows (#1619). **Three breaking behavior changes.** Mission-config schema **2.0** retires the `expand` primitive, and because `TaskConfig` carries a `serde(flatten)` overflow a config still declaring it would have parsed cleanly and silently lost its fan-out, so `validate` now errors naming the field and the migration (#1550); a dangling `role_profiles` binding errors instead of silently falling back to `default_profile` (#1547); and the serve daemon's auth gate **fails closed**: a request with no peer address is treated as remote rather than as loopback, so the whole remote gate no longer rests on a single wiring call guarded by a comment (#1663). Loopback-only installs are unaffected. `FLOW_SCHEMA` stays 1.18.0; existing audit chains survive without rotation. **v2.4.0, the observability release: CLI panels in the browser, and the data underneath them made honest.** A `console` lens renders real `darkmux` command output as styled DOM off an allowlisted `GET /panel/:id`: the CLI stays the single source of truth and the viewer renders it, rather than reimplementing it into the twin-drift #1561 already was (#1569). Mission ids become clickable via OSC 8 terminal hyperlinks, four run tabs collapse to three (#1584), and phases render as containers in the graph (#1594). The board row now says what a mission **is**: it led with the mint id (`dispatch-code-reviewer-1785589698-5d6a-0`), which on a phone ate two thirds of the width for the least informative thing on the line; rows carry a name, a graph-vs-role glyph, a short discriminator and an age instead, with the id one click away (#1612). Underneath, a batch of producer-side defects that made the data quietly wrong: a dispatch preflight could **unload a model you owned** (#1609); a namespaced `internal.utility` binding made the **compactor unloadable**, so compaction silently fell back to a ~4096-token JIT load with truncated summaries (#1615); a **starved judge grant deleted a real finding** while the call "succeeded" so no degraded gate fired (#1610); a newer peer's flow record read as **chain corruption**, a false tamper alert on the compliance substrate (#1611); and `dispatch.map` emitted no liveness bookends, so a production path doing model work was invisible while it ran (#1607). Plus the phone: a 390px screen fits ~52 columns but both ends of the negotiation floored at 60, hanging nine columns off the edge (#1613/#1614), and a review read `0/3` for its **entire run** because phases never closed until finalize (#1620). `FLOW_SCHEMA` stays 1.18.0; existing audit chains survive without rotation. **v2.3.1: a degraded Redis no longer takes the dashboard with it.** Only the *connect* phase of a Redis read was bounded, so a peer whose TCP port accepted but which never answered a command blocked until the route's 30s timeout returned `408`, with the local-file fallback unreachable, because a hang is not an error (#1570). A response deadline now bounds the command, and the local read runs **concurrently** rather than behind an optional network hop. Separately, a *recovered* Redis used to **erase history**: its result replaced the local file wholesale, but Redis rides a `MAXLEN` cap and is missing everything written while it was unreachable, so the outage window vanished from the view the moment it came back. The two are now unioned. Plus three viewer-honesty fixes: no scrubber flash before the mode is known (#1565), a visible `reconnecting` state with auto-refetch and a manual refresh control for the home-screen app (#1480), and an idle headline that stops hiding its own recency. **v2.3.0, composable mission graphs: store review variants and launch them by name.** Step kinds became stateless singletons in one global registry, and launchers now route on what a graph **declares** rather than what it is **named**, so `~/.darkmux/mission-configs/review-lean.json` (fewer probe seats, a different judge depth, a different model per role) runs with `darkmux mission launch review-lean`, and the launched document is the one that executes (#1530). Every pipeline now produces its own input **inside** the graph instead of in a bespoke pre-launch prelude (bundling, probe collections, and the coder brief are all runtime graph work), which is what makes a graph extensible from either end (#1540/#1545/#1546). Step kinds declare the run-scoped artifacts they require, so a graph that can't compose **fails up front naming the artifact, the kinds that need it, and how to supply one**, instead of panicking mid-run after the mission had already minted (#1556). Per-seat probe prompts (`review-probe-high.md`/`-mid.md`/`-low.md`) are live; the files existed and were silently ignored. Plus the fixes a four-variant release dogfood surfaced: the dashboard is served with cache validators (a reload revalidates in **zero bytes** instead of re-fetching ~256 KB, and can never be stale), map-dispatched work reports its prompt/completion split so the fleet card's chips stop under-reporting **41%** of local tokens against their own headline, and the 24h activity window no longer reads `11:38:41–11:38:41`. No schema bumps: a 2.2 install upgrades in place and a 2.2 peer stays wire-compatible. **v2.2.0: a unified runs read-model, a lighter machine page, honest remote-run display, and tighter reviews.** `/runs` lands as one flat, kind-tagged read-model that unions missions, lab runs, and flow into a single per-request view, the data layer for a coming Runs lens, read-side only, no new store (#1523). The machine tab and the fleet drill-down converge on **one lightweight page**: a live residency/RAM health region plus a runs list, honest about local-vs-remote (a machine probed elsewhere reads "not reported from here", #1522). A run served off-fleet on a hosted endpoint **no longer shows the box's incidental local LMStudio residency as its model**; route and model resolve from the run's own endpoint (#1518). Review comments render the "needs frontier verification" note **once per review** instead of on every finding (#1528), and a review's dispatch bookends now carry their `mission_id` so a review shows as exactly one row in `/runs` (#1523). No schema bumps; a 2.1 install upgrades cleanly. **v2.1.0: config-composable review + concurrency-safe residency.** The review pipeline becomes pure config-composed primitives: probes are explicit one-role tasks, so the probe **count is config-driven** (compose a lean one-probe review for a 32 GB tier), step kinds are swappable, and there's no hard-coded "probe role" concept (#1513). The residency arbiter now **evicts stale orphans and protects a concurrent command's in-use model** via per-pid leases, so heavy cross-family crews run without a manual `machine eject` (#1487). And `darkmux dispatch` is now a **first-class run through the same engine** as missions: it shows up in `mission status`, mints a run record, and joins the lease/reconcile regime (#1510); a mission run is a first-class record with a unique run id (#1503). No schema bumps; a 2.0 install upgrades cleanly. See the CHANGELOG for the behavior-change notes (dispatch-as-run, relaunch-fresh, `--k>1` retired). **v2.0.0: the mission orchestrator.** Config-defined **missions** launched with `darkmux mission launch <config>` run as a live **Task/Step dependency graph**: a crew of local-AI (or hosted) seats works the phases through the internal Docker-bounded runtime, concurrent local dispatch is bounded by a residency arbiter that loads exactly what each seat's staffing declares, and a React Flow **mission-graph lens** draws the graph lighting up as it runs. The PR-review funnel (`mission launch review`) and the coder pipeline (`mission launch coder-phase`) both became missions; residency (the founding profile-multiplexer) is now an internal capability, not a verb. The verb surface shrank hard (`dispatch` and `machine` are the top-level families; `swap`/`pr-review run`/`crew sync`/the openclaw shell-out path all retired), review seats staff from a machine-local **role→profile map**, and `mission close` → `mission finalize`. A **major** release with no deprecation shims; see the CHANGELOG's Migration table. `FLOW_SCHEMA` 1.17.0→1.18.0, `CONFIG_SCHEMA`→1.5 (additive, lenient-read). **v1.18.5** was two fixes surfaced by a real production 37-flag funnel run: the run-level `degenerate` gate no longer over-fires on a minority remote-judge dispatch error (a single flag's transient failure out of 37 was discarding the whole run's real, valid findings: 9 confirmed + 9 needs-check lost, CI false-alarmed on a run that actually completed correctly, #1329); and funnel provenance now stamps the model an endpoint actually **served**, not just the requested deployment name, so an aliased Azure deployment is now surfaced in the posted footer instead of silently hidden (#1300). **v1.18.4** was two fixes surfaced by running the funnel on a private production repo: config.json now resolves to **user scope**, never a project-local shadow (a stray project `.darkmux/` on a self-hosted-runner checkout had been silently disabling Redis + the **audit log** on every review dispatch, #1323); and the review footer no longer claims "darkmux dogfooding itself in public" on private repos. **v1.18.3** was a one-fix patch: the review funnel's confirmed findings now **anchor as inline comments** instead of falling into the summary's general section (the mis-anchor half of #1299: `resolve_anchor` gains a substring fallback so a quoted sub-expression resolves to its diff line; a real preserved review envelope replayed 0 inline / 7 general → 6 inline / 1 general). **v1.18.2** was the **production-hardening patch**: a memory-ledger correctness fix (prices models whose LMStudio path metadata is wrong, #1309) plus the credential-and-hang surface the first Azure-review day surfaced: a dependency-free **dispatch liveness floor** so a hang before flow init isn't a black box (#1311), **bounded Keychain reads** (a locked headless-runner keychain fails fast, not a multi-minute freeze), and **`EndpointAuth.key_env`** to pull an endpoint's key from an env var so a self-hosted runner never touches the Keychain (#1312, `PROFILES_SCHEMA` 1.4→1.5). v1.18.1 was the **review-output patch**: everything the first real production Azure review surfaced about how the funnel *presents* its findings (no change to what it finds). Confirmed findings now post a **non-blocking `COMMENT`** review (the same class Gemini uses, inline comments intact) instead of a `REQUEST_CHANGES` merge gate darkmux couldn't clear itself (#1302); the posted footer names where seats **actually ran** rather than falsely claiming "no cloud API" on a cloud review (#1298); frontier-worded **duplicate findings collapse** and the needs_check tier clusters (#1299); and a **`passes: 1` judge knob** cuts frontier cost (#1266). `PROFILES_SCHEMA` 1.2→1.4 (additive `passes` + `request_changes`). v1.18.0 was the **frontier-staffing release**: any review-funnel seat can be staffed by a hosted model, so a machine with LMStudio shut down does PR review entirely off a cloud endpoint (remote seats + an optional `review-verify` adjudication stage + per-execution token buckets, #1260/#1177). It also ships a **memory ledger** showing each loaded model's committed-vs-materialized RAM (`darkmux model ledger`, `/machine/memory`, the `#lens=machine` view, #1286) and a new model-lifecycle core (a pure planner, wave scheduler, architecture-aware KV estimator, and deadline-bounded `lms` adapter), present but not yet wired to the live dispatch path (that cutover lands in a later 1.18.x, #1274/#1276/#1285). `PROFILES_SCHEMA` 1.1→1.2 (optional `n_ctx` for endpoint models, #1282). v1.17.1 was the canary-day patch (registry blast-radius, cycler residency reconciliation, funnel dispatch visibility; #1269/#1271/#1272). v1.17.0 is the **review-funnel release**: PR review becomes a measured prosecution-and-judgment pipeline: `darkmux pr-review run` bundles the change (callee/sibling bodies + mechanical facts), fans strong-prior probe seats across it, dedups by mechanism, double-confirms every finding with an independent judge pass, and posts a three-tier review (blocking confirmed comments carrying a "needs frontier verification" marker, a non-blocking worth-a-double-check section, everything archived in the envelope artifact), staffed by **crews** saved in `profiles.json` (#1222). The lab grew the observability to tune it: per-run event streams, host telemetry, staffing snapshots, and the viewer's third lens: the machine-local **lab observer** with run series, knob-diff provenance, and live run detail (`#lens=lab`, #1247). Funnel prompts are byte-locked to the measured reference implementation by golden tests (#1256). `FLOW_SCHEMA` 1.15.0→1.17.0 (additive), `PROFILES_SCHEMA` 1.1 (additive `crews`). v1.16.0 was the **production-review release**: a freeform review contract that works WITH tools (`MUST FIX`/`CONSIDER [path] \`anchor\`` markers + a `VERDICT:` line; a grammar-constrained `output_schema` combined with tools makes a model skip tool-calling and fabricate, so the agentic reviewer writes markers instead, #1192), the `pr-reviewer-agentic` role (repo checked out, read/exec tools, explores callers and siblings before concluding, #1192), `darkmux doctor --probe` (live-verifies a remote endpoint credential with one minimal completion through the exact dispatch path: shows round-trip, the model the endpoint says served it, and the probe's own token cost, #1191), `pr-review render` `mode: degraded` so a produced-nothing review can never read as a green check (#1193), and the live viewer's **local/cloud token split**: paid cloud tokens no longer count as "off the meter"; the hero shows local tokens and cloud tokens as two honest numbers (#1189). No schema changes. v1.15.0 added **agentic-remote dispatch**, a tool-granting role (e.g. `code-reviewer`) driven by a remote OpenAI-compatible endpoint (Azure OpenAI, OpenAI, …) as its "brain," running the same real tool-calling loop local models get via the internal container runtime, not just a single-shot chat completion (#92), plus a `darkmux doctor` check for remote-endpoint credential presence (#85/#91) and a viewer host-load meter (#1064). `FLOW_SCHEMA` 1.14.0→1.15.0 (additive host-load telemetry fields; older binaries tolerate it). v1.14.1 was a viewer performance hotfix for a long-open live tab (memoized session lookups + incremental SSE reconcile, #1173). v1.14.0 adds cross-day **playback discoverability** (a mission/dispatch catalog + `?mission=`/`?session=` replay-by-query so you replay by the thing that ran, not by calendar day, #691) and overhauls the run-detail **telemetry panel**: CPU and context on one wall-clock axis, a context step-area with per-turn markers + a fullness gradient + a compaction-threshold line, CPU shown in cores-busy, and the `model (lms)` telemetry section fixed (#1169). Drop-in over 1.13.1 (no schema change). v1.13.1 was a stability patch from a review-swarm audit of the recently-shipped code: compaction no longer orphans a tool-result at the tail boundary (an opaque HTTP 400 that hard-failed otherwise-productive dispatches, #1158), a dispatch panic can't silently kill the fleet runner (#1159), a job published before a runner exists isn't lost (#1160), `/diff` offloads to the blocking pool and streams git output under its 256KB cap (#1161), and the live viewer stops leaking per-session ids on a long-lived tab (#1162). No schema changes, so it drops in over a v1.13.0 install. v1.13.0 was the fleet-foundation + self-diagnosing-doctor release: declare a machine's fleet position (`fleet.mode` hub/peer/standalone, #933), set config from the CLI instead of hand-editing JSON (`darkmux config set/get/list`, #937), and `darkmux doctor` now catches cross-setting traps (a stale env var shadowing config, a brew/cargo split-brain) + leads with an `● ok / needs attention / broken` verdict (#934) + shows where to open the viewer (loopback + tailnet, #1155), plus a live-view UX pass: tokens-in/out accumulate live, an SSE backstop surfaces new runs without a refresh, mobile layout fixes, and `10m/1h/4h/24h` activity-timeline presets (#1151). No `FLOW_SCHEMA` change (stays `1.14.0`, cross-machine compatible with a v1.12.0 peer); `CONFIG_SCHEMA` 1.0→1.1 (additive `fleet{}` block, lenient-read). v1.12.0 was a build-visibility + run-observability + production-hardening release: build identity in `darkmux --version` / `doctor` / the viewer header (with a Homebrew `(release)` tag, #1129), a clearer run drill-down page (status pill + `run · <role>`, a run brief, tokens in/out tiles, the dispatch prompt + container image, #1125/#1127/#1126), an "about" modal off the version chip (#1132), `darkmux doctor` issues-only by default (#1130), and the production-hardening fixes surfaced by darkmux's first brew-stable user: `dispatch` now loads the model at the profile's `n_ctx` instead of LMStudio's silent default (#1135), the runtime recovers from an empty `finish_reason=tool_calls` completion instead of hard-failing (#1123), and internal-path dispatch errors carry the stderr text so a remote failure is diagnosable from the flow stream (#1042). v1.11.2 was a bug-fix + accessibility + security patch from a board triage: live "in flight" now derives from presence so a hard-killed dispatch stops reading "running" forever (#857), a truthful/de-duplicated live status line + consolidated headline (#1103/#1105), dispatch-error records that carry the stderr text (#1042), keyboard navigation + a non-color status cue for the viewer (#1090/#1092), and argument-injection guards on `gh pr create --label` and `external pull` (#1111/#1112). v1.11.1 was a focused viewer + UX pass (a cleaner, mostly-mobile dashboard with one status-color convention everywhere: green = success, yellow = running, orange = canceled, red = failed/killed, #1071; icon-first chrome, #1067/#1098; local-timezone timestamps, #1069; and a full-width savings hero on the missions tab, #1096), plus a local-PR-reviewer reliability fix (the role no longer copies its own worked example, #1084). v1.11.0 made the local **PR reviewer** materially better and self-contained: it assesses each change against its **stated intent** (the PR title + description) so it stops flagging the very bug a fix removes (#1053), anchors findings by **quoting the line** and resolving that quote to a coordinate in the harness (local models name the construct well but guess line numbers badly, #1053), and moves the whole review render into the binary as `darkmux pr-review render`, versioned with the role schema instead of a per-repo script (#1060). darkmux also reviews its own PRs in public, on a local model, via a self-hosted runner (#1047/#1057), and `dispatch --profile <name>` lets a caller name a profile the machine resolves (#1054). v1.10.0 turned a local model into an automated **PR reviewer** (the tool-less `pr-reviewer` role + grammar-constrained `output_schema` so a small model can't emit malformed JSON, #1037/#1039/#1044). v1.9.0 completed the engagement-context cure: staleness-aware cautions (#1001/#1002), a fraction-of-window injected-context budget (#1011), `darkmux lessons edit/remove/export/import/recall` curation (#1003), and `darkmux lab loop --ab` for the verdict-shift A/B (#1004), plus `dispatch --message-from-file` (#386). v1.8.0 laid the foundation (the cautions + lessons store + `mission debrief`, #994/#1000); v1.7.0 added the loop lab (#986); v1.6.0 persisted corrections into the next brief (#849); v1.5.0 added the verifier-fabrication gate (#799). The dispatch-to-PR loop, the missions lens + `mission status`, live observability, and the Homebrew pipeline are the stable surface.

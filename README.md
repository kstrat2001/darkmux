...
✅ **v1.10.0**, semver-stable. v1.10.0 turns a local model into an automated **PR reviewer**: a tool-less `pr-reviewer` role reviews a diff and emits a structured, cite-the-line JSON review for CI to post as native inline comments (#1037), and the runtime can now **grammar-constrain** any role's output to a declared `output_schema` (passed to LMStudio as `response_format`), so a small local model can't emit malformed JSON (#1039) — findings carry both prose `advice` and a clean-apply one-click `suggestion` (#1044). v1.9.0 completed the engagement-context cure: staleness-aware cautions (#1001/#1002), a fraction-of-window injected-context budget (#1011), `darkmux lessons edit/remove/export/import/recall` curation (#1003), and `darkmux lab loop --ab` for the verdict-shift A/B (#1004), plus `crew dispatch --message-from-file` (#386). v1.8.0 laid the foundation (the cautions + lessons store + `mission debrief`, #994/#1000); v1.7.0 added the loop lab (#986); v1.6.0 persisted corrections into the next brief (#849); v1.5.0 added the verifier-fabrication gate (#799). The dispatch-to-PR loop, the missions lens + `mission status`, live observability, and the Homebrew pipeline are the stable surface.

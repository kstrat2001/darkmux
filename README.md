...
✅ **v1.9.0**, semver-stable. v1.9.0 completes the engagement-context cure: detector firings now carry a BLAKE3 hash of the file they fired on, so a caution about since-changed code ranks **down** as stale, and cautions/lessons about a file the dispatch will touch rank **up** (#1001/#1002); the injected context is **budgeted as a fraction of the model's window** with per-authority floors instead of flat counts (#1011); `darkmux lessons edit/remove/export/import/recall` give full curation of the store (#1003); and `darkmux lab loop --ab` runs the same task with vs without the injected memory and reports the **verdict shift** — the empirical proof (#1004). Plus `crew dispatch --message-from-file` for substantial briefs, passed via a mount rather than `docker run` argv (#386). v1.8.0 laid the foundation (the cautions + lessons store + `mission debrief`, #994/#1000); v1.7.0 added the loop lab (#986); v1.6.0 persisted corrections into the next brief (#849); v1.5.0 added the verifier-fabrication gate (#799). The dispatch-to-PR loop, the missions lens + `mission status`, live observability, and the Homebrew pipeline are the stable surface.

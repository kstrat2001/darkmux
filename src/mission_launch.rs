...
pub fn launch(
    ...
    let (mut tasks, mut all_steps, interpret_warnings) =
    ...
    // persist loop just below, so a static unit's on-disk record carries
    // the same stamp timing a grown unit's record already gets (its own
    // stamp runs before ITS mint-time persist too — see the `grow_phase`
    // call site).
    ...
    //
    // (#2595 review round 2, MUST FIX 1) The coder step's OWN stamp
    // (`register_coder_phase_kinds`) targets an INERT field
    // (`DispatchOpts::timeout_seconds` — see that function's own doc), so
    // collapsing the flag to a plain `u32` here is safe for that one
    // consumer. `grow_phase`'s crawl/review UNIT stamp is a SEPARATE
    // consumer of a LIVE field and must see the RAW `Option` instead — see
    // `stamp_unit_timeout`'s own doc for why collapsing it the same way
    // here was the bug. Do not reuse `resolved_timeout_seconds` for
    // `grow_phase`; pass the original `timeout_seconds` param.
    let resolved_timeout_seconds = timeout_seconds.unwrap_or(600);
...
/// doc on that field), the ONE field `dispatch_internal::effective_
/// inactivity_timeout_seconds` reads to decide the dispatch's real
/// inactivity budget — a LIVE destination, unlike the coder step's own
/// stamp below (`register_coder_phase_kinds`, whose destination —
/// `DispatchOpts::timeout_seconds` — bounds only the tool-less single-call
/// paths a container-agentic coder dispatch never takes, so collapsing to a
/// plain `u32` there can never claw back anything real).
///
/// Growth (not `register_coder_phase_kinds`) is the right place for a
/// crawl/review unit specifically because a unit `Step` doesn't exist until
/// its phase boundary grows it (`grow_task` below) — there is no static
/// `Step.config` to stamp before then, the way there is for the coder step.
///
/// **The rule: `None` stamps NOTHING; `Some(n)` OVERWRITES unconditionally
/// — never "fill only if absent".** An earlier version of this function
/// collapsed the flag to a `u32` with `.unwrap_or(600)` before reaching
/// here, so an *omitted* flag was indistinguishable from a typed
/// `--timeout 600` and got stamped onto every grown unit regardless. Two
/// live consequences that fix undid: an operator's own `config.runtime.
/// inactivity_timeout_seconds` / `DARKMUX_INACTIVITY_TIMEOUT_SECONDS` was
...
/// it (absent stays absent, so the standing env/config/built-in resolution
/// decides and reports its own real tier); an explicit flag wins OUTRIGHT,
/// overwriting even a hand-authored per-rule `timeout_seconds` a task's
/// `grow.config` or static `config` already declared — the exact same
/// "CLI wins outright, not merely fills a gap" rule `effective_
/// inactivity_timeout_seconds` already applies one layer down for every
/// other `dispatch --timeout` call. A crawl-unit template that wants a
/// timeout an operator's `--timeout` can't clobber has no way to express
/// that today, same as any other `dispatch --timeout` caller — consistent,
/// not a regression.
fn stamp_unit_timeout(steps: &mut BTreeMap<String, crew::types::Step>, timeout_seconds: Option<u32>) {
    let Some(timeout_seconds) = timeout_seconds else {
        // No explicit --timeout: touch nothing. Leaving every grown unit's
        // config exactly as the template produced it is the fix itself —
        // see this function's own doc for what writing the collapsed
        // 600-default in this branch used to break.
        return;
    };
    for step in steps.values_mut() {
        ...
        // template-declared per-rule value. Never "fill only if absent":
        // that was the bug (see this function's own doc).
        let is_null = step.config.is_null();
        match step.config.as_object_mut() {
            ...
            // loudly naming the missing `plan`/`unit` fields, which is a
            // far better failure than silently destroying non-null,
            // non-object config data.
            None => {}
        }
    }
}
...
fn grow_phase(
    ...
    collected: &BTreeMap<String, serde_json::Value>,
    // (#2595 review round 2, MUST FIX 1) The RAW `--timeout` flag — see
    // `stamp_unit_timeout`'s own doc for why this must stay an `Option`
    // rather than the caller's collapsed `.unwrap_or(600)` value.
    timeout_seconds: Option<u32>,
...
/// The honest per-phase rules, applied to the phase's TASK-collapsed
/// statuses (see [`phase_finalization`] for why the grain is the task, not
/// the raw step):
///
/// - all of the phase's tasks `Complete` (and it has at least one) → Complete
/// - every task terminal, SOME `Complete` AND some `Error`/`Abandoned` →
///   **Degraded (#2406)** — real output shipped, some of it did not; NOT the
///   same as Abandoned, and NOT a lesser Complete either (see
///   [`crew::envelope::PhaseOutcomeKind`]'s own doc)
...
/// - any work left non-terminal (`Running`/`Planned`) → Abandoned (this
...
/// Abandoned regardless of how many OTHER tasks in the same phase
/// completed — a phase with 11 completed units and 1 errored one read
...
/// (#2406, post-review) Collapse ONE task's step statuses into that task's
/// status, with the scheduler's own "a task is one unit of work" rule:
/// any `Error` wins, else any `Running`, else all-`Complete`, else any
/// `Abandoned`, else `Planned`. Byte-for-byte the same rule as the display
/// side's `crates/darkmux-serve/src/mission_graph.rs::derive_task_status`
/// — the two must stay in lock-step, which is the whole point of
/// [`phase_finalization`] collapsing at this grain.
fn task_status_from_steps(step_statuses: &[NodeStatus]) -> NodeStatus {
...
///
/// **The grain is the TASK, not the raw step (#2406 post-review).** The
/// steps are grouped by `Step::task_id` and each group collapsed by
/// [`task_status_from_steps`] BEFORE the mix rule is applied. Counting raw
/// steps applied the mix rule one level too low — the mirror image of the
/// bug #2406 fixes, which applied the collapse one level too high:
///
/// - The issue itself specifies the grain ("1 of 12 unit **tasks**
///   errored… task level keeps 'any Error wins'"). A phase is a set of
///   INDEPENDENT tasks; a task's steps are STAGES of one unit, not
///   independent deliverables.
/// - So a single task whose steps are `[Complete, Abandoned]` is a
///   half-finished unit, not "real output shipped, some of it did not".
...
///
/// One thing deliberately stays at STEP grain: `any_started`. "The
/// scheduler never reached this phase" is a statement about whether any
/// step ever left `Planned`, and a task collapsing to `Planned` (e.g.
/// `[Complete, Planned]`) would otherwise be misread as never-started.
fn phase_finalization(phase_steps: &[&crew::types::Step]) -> (crew::envelope::PhaseOutcomeKind, Option<String>) {
    ...
    let mut by_task: BTreeMap<&str, Vec<NodeStatus>> = BTreeMap::new();
    for s in phase_steps {
        by_task.entry(s.task_id.as_str()).or_default().push(s.status);
    }
    let task_statuses: Vec<NodeStatus> =
        by_task.values().map(|steps| task_status_from_steps(steps)).collect();
    let total = task_statuses.len();
    let completed = task_statuses.iter().filter(|s| **s == NodeStatus::Complete).count();
    let errored = task_statuses.iter().filter(|s| **s == NodeStatus::Error).count();
    let abandoned = task_statuses.iter().filter(|s| **s == NodeStatus::Abandoned).count();
    let non_terminal = task_statuses
        .iter()
        .filter(|s| matches!(s, NodeStatus::Running | NodeStatus::Planned))
        .count();
    // Step grain on purpose — see this fn's own doc.
    ...
    } else if non_terminal == 0 && completed > 0 && (errored > 0 || abandoned > 0) {
        // (#2406) The fix: a mix of Complete and Error/Abandoned among a
        // TERMINAL phase's own tasks is Degraded, not Abandoned — real
        // output shipped, some of it did not. This is the ONLY branch
        // #2406 changes; every other arm below is byte-identical to the
        // pre-#2406 rule.
        //
        // The `non_terminal == 0` guard is defensive, not decorative
        // (#2406 post-review). This fn's own doc says callers guard
        // terminality, and `lazy_close_prior_phases` genuinely does
        // (it `continue`s on any `Running`/`Planned` step) — but
        // `derive_phase_outcomes` does NOT. Without the guard, a phase
        // holding `[Complete, Error, Planned]` reached this branch and
        // returned `Degraded`, which routes to `phase_complete` and
        // CLOSES a phase that still has planned work. Every other arm
        // already tolerates non-terminal input; this new one has to as
        // well, and the leftover falls through to the non-terminal arm
        // at the bottom exactly as it did before #2406.
        (
...
mod tests {
    ...
    fn phase_finalization_table_every_combination() {
        use crew::envelope::PhaseOutcomeKind;
        let s = scripted_step;
        // (status list, expected outcome, expected reason, human name) —
        // one row per reachable combination of terminal/non-terminal step
        // statuses a phase's own step slice can carry. #2406 changes ONLY
        // the "mixed Complete + Error/Abandoned, all terminal" rows
        // (marked below); every other row asserts the PRE-#2406 behavior
        // is unchanged (the inverted cases the brief asks for).
        //
        // `scripted_step` gives every step its own `task_id`, so each row
        // below is N single-step tasks — the grain `phase_finalization`
        // collapses to. The MULTI-step-task shapes (where the collapse is
        // what distinguishes this from raw step counting) are pinned
        // separately in
        // `phase_finalization_collapses_a_multi_step_task_before_the_mix_rule`.
        let cases: Vec<(Vec<NodeStatus>, PhaseOutcomeKind, Option<&str>, &str)> = vec![
            // all-complete → Complete (inverted case: unaffected by #2406)
            (vec![NodeStatus::Complete, NodeStatus::Complete], PhaseOutcomeKind::Complete, None, "all complete"),
            (vec![NodeStatus::Complete], PhaseOutcomeKind::Complete, None, "single complete"),
            // nothing complete, all errored → Abandoned (inverted case:
            // "nothing complete" stays Abandoned, unaffected by #2406)
            (
                vec![NodeStatus::Error, NodeStatus::Error],
                PhaseOutcomeKind::Abandoned,
                Some("2 task(s) errored"),
                "all errored",
            ),
            (
                vec![NodeStatus::Error],
                PhaseOutcomeKind::Abandoned,
                Some("1 task(s) errored"),
                "single errored",
            ),
            // nothing complete, all abandoned, no errors → Abandoned.
            //
            // SURPRISE, recorded rather than encoded silently: the REASON
            // on this row reads "steps left non-terminal", which is false
            // — every step here IS terminal. It falls out of the arm
            // ordering (nothing complete, nothing errored, something
            // started ⇒ the catch-all), is UNCHANGED by #2406, and is
            // cosmetic only (the OUTCOME, which is what drives the
            // lifecycle and the debrief, is correct). Pinned here so the
            // wording is on the record instead of being discovered again;
            // fixing it is a separate, behavior-free change.
            (
                vec![NodeStatus::Abandoned, NodeStatus::Abandoned],
                PhaseOutcomeKind::Abandoned,
                Some("phase did not complete (steps left non-terminal)"),
                "all abandoned",
            ),
            ...
            (
                vec![NodeStatus::Planned, NodeStatus::Planned],
                PhaseOutcomeKind::Abandoned,
                Some("phase never started (scheduler did not reach it)"),
                "never started",
            ),
            (
                vec![],
            ...
            // ── NON-TERMINAL beside a BAD terminal — the gap the table's
            // own name used to claim to cover and did not. These are the
            // rows that guard #2406's new Degraded branch against firing on
            // a phase that is not over: without the branch's
            // `non_terminal == 0` guard, the first row below returns
            // Degraded, which routes to `phase_complete` and CLOSES a phase
            // still holding a Planned task. `derive_phase_outcomes` (unlike
            // `lazy_close_prior_phases`) does not guard terminality, so the
            // input is reachable.
            (
                vec![NodeStatus::Complete, NodeStatus::Error, NodeStatus::Planned],
                PhaseOutcomeKind::Abandoned,
                Some("1 task(s) errored"),
                "complete + error + STILL PLANNED — never Degraded, the phase is not over",
            ),
            (
                vec![NodeStatus::Complete, NodeStatus::Error, NodeStatus::Running],
                PhaseOutcomeKind::Abandoned,
                Some("1 task(s) errored"),
                "complete + error + STILL RUNNING — never Degraded",
            ),
            (
                vec![NodeStatus::Complete, NodeStatus::Abandoned, NodeStatus::Planned],
                PhaseOutcomeKind::Abandoned,
                Some("phase did not complete (steps left non-terminal)"),
                "complete + abandoned + still planned — nothing errored, so the leftover arm",
            ),
            (
                vec![NodeStatus::Error, NodeStatus::Running],
                PhaseOutcomeKind::Abandoned,
                Some("1 task(s) errored"),
                "errored + still running, nothing complete",
            ),
            // ── #2406's fix: terminal, SOME complete + SOME error/abandoned ──
            (
                vec![NodeStatus::Complete, NodeStatus::Error],
                PhaseOutcomeKind::Degraded,
                Some("1 of 2 task(s) completed, 1 errored, 0 abandoned"),
                "complete + error (THE #2406 bug shape)",
            ),
            (
                vec![NodeStatus::Complete, NodeStatus::Abandoned],
                PhaseOutcomeKind::Degraded,
                Some("1 of 2 task(s) completed, 0 errored, 1 abandoned"),
                "complete + abandoned",
            ),
            (
                vec![NodeStatus::Complete, NodeStatus::Complete, NodeStatus::Error, NodeStatus::Abandoned],
                PhaseOutcomeKind::Degraded,
                Some("2 of 4 task(s) completed, 1 errored, 1 abandoned"),
                "complete + error + abandoned, all mixed",
            ),
        ];
        for (statuses, expected, expected_reason, name) in cases {
            ...
            assert_eq!(reason.as_deref(), expected_reason, "case `{name}`: reason for {statuses:?}");
            assert_eq!(outcome, expected, "case `{name}`: statuses {statuses:?}");
        }
    }

    #[test]
    fn phase_finalization_collapses_a_multi_step_task_before_the_mix_rule() {
        // (#2406, post-review) The envelope and the mission-graph lens must
        // not describe the same phase with different words. They did: this
        // fn used to count RAW STEPS, while the lens counts TASKS
        // (`mission_graph.rs::derive_task_status` → `phase_task_rollup`),
        // so a mix living INSIDE ONE multi-step task was destroyed a level
        // too low on one side and not the other.
        //
        // Fixed by collapsing per task here, at the grain the issue itself
        // specifies ("1 of 12 unit tasks… task level keeps 'any Error
        // wins'"). Its TWIN on the display side is
        // `mission_graph.rs::phase_display_of_a_single_task_with_mixed_steps_agrees_with_the_envelope`;
        // the two fixtures and expectations are written to match.
        fn step_in_task(id: &str, task_id: &str, status: NodeStatus) -> crew::types::Step {
            let mut st = scripted_step(id, status);
            st.task_id = task_id.to_string();
            st
        }
        use crew::envelope::PhaseOutcomeKind;
        ...
        // DISK said `complete`) — three words for one phase, and that
        // disagreement was NEW in #2406: pre-#2406 both sides said
        // `Abandoned`.
        let steps = [
            step_in_task("s1", "t-one", NodeStatus::Complete),
            step_in_task("s2", "t-one", NodeStatus::Abandoned),
        ];
        let refs: Vec<&crew::types::Step> = steps.iter().collect();
        assert_eq!(
            phase_finalization(&refs).0,
            PhaseOutcomeKind::Abandoned,
            "one task's [Complete, Abandoned] steps collapse to ONE abandoned task — never a phase-level mix"
        );

        // ONE task, steps [Complete, Error] → the task is Error, so the
        // phase is not a mix either. `PhaseOutcomeKind` has no `Error`
        // variant (documented, unchanged by #2406), so an all-errored
        // phase spells its terminal `Abandoned` — the display side's
        // `error` and this `Abandoned` are that ONE documented vocabulary
        // difference, exactly as they were before #2406, not a new
        // divergence.
        let steps = [
            step_in_task("s1", "t-one", NodeStatus::Complete),
            step_in_task("s2", "t-one", NodeStatus::Error),
        ];
        let refs: Vec<&crew::types::Step> = steps.iter().collect();
        let (outcome, reason) = phase_finalization(&refs);
        assert_eq!(outcome, PhaseOutcomeKind::Abandoned);
        assert_eq!(reason.as_deref(), Some("1 task(s) errored"), "one errored TASK, not two steps");

        // The collapse must not swallow #2406's actual fix: the SAME six
        // step statuses, split across TWO tasks (one clean, one failed),
        // are a genuine phase-level mix and still read Degraded.
        let steps = [
            step_in_task("s1", "t-ok", NodeStatus::Complete),
            step_in_task("s2", "t-ok", NodeStatus::Complete),
            step_in_task("s3", "t-bad", NodeStatus::Complete),
            step_in_task("s4", "t-bad", NodeStatus::Error),
        ];
        let refs: Vec<&crew::types::Step> = steps.iter().collect();
        let (outcome, reason) = phase_finalization(&refs);
        assert_eq!(outcome, PhaseOutcomeKind::Degraded, "one clean task + one failed task IS a phase-level mix");
        assert_eq!(reason.as_deref(), Some("1 of 2 task(s) completed, 1 errored, 0 abandoned"));
    }
    ...
    fn phase_finalization_the_2406_scenario_1_errored_11_complete_is_degraded_not_abandoned() {
        ...
        // tasks errored, 11 completed. The pre-#2406 rule mapped this
        // straight to Abandoned ("N step(s) errored") regardless of how
        // many other units in the phase completed — the debrief then called
        // a phase that shipped 11 real completions "abandoned". Fixed: the
        // phase reads Degraded, and the reason names both counts.
        //
        // `scripted_step` gives every step its OWN `task_id`, so these 12
        // steps are 12 independent tasks — which is what the issue's own
        // wording describes ("unit tasks") and the grain
        // `phase_finalization` collapses to.
        ...
        assert_eq!(reason.as_deref(), Some("11 of 12 task(s) completed, 1 errored, 0 abandoned"));
    }
    ...
    // ─── #2595 review round 2: `--timeout`'s OMITTED half must not clobber
    //     the standing env/config/600 tier, and its EXPLICIT half must win
    //     outright ────────────────────────────────────────────────────────
    //
    ...
    // identical to an explicit `--timeout 600` and silently overrode the
    // operator's own `config.runtime.inactivity_timeout_seconds`/env
    // setting on every default launch — these tests pin the corrected
    // `Option<u32>` shape: `None` touches nothing, `Some(n)` overwrites
    // unconditionally (never "fill only if absent"). Both `stamp_unit_
    // timeout` directly and one full pass through `grow_phase` are pinned
    // so a regression at the call site — forgetting to call it, calling it
    // against the wrong map, or re-collapsing the `Option` upstream — is
    // caught too.
    ...
    // ── the `None` arm (the flag was OMITTED) — must touch NOTHING ──

    #[test]
    fn stamp_unit_timeout_omitted_flag_leaves_an_absent_value_absent() {
        // (#2595 review round 2, MUST FIX 1, red-prove) This is the exact
        // reproduction of the reviewer's probe: a grown unit whose template
        // never named `timeout_seconds` must still have NO
        // `config.timeout_seconds` after an omitted-flag launch, so the
        // standing `env(DARKMUX_INACTIVITY_TIMEOUT_SECONDS)` / `config.
        // runtime.inactivity_timeout_seconds` / built-in-600 resolution
        // stays live downstream (`UnitStepConfig::from_step` ->
        // `DispatchOpts::timeout_override_seconds` ->
        // `effective_inactivity_timeout_seconds`). Before this fix, this
        // exact call used to write `Some(600)` here regardless.
        let mut steps = BTreeMap::new();
        steps.insert("u1".to_string(), crawl_unit_step("u1", serde_json::json!({"plan": "p", "unit": "u1"})));
        stamp_unit_timeout(&mut steps, None);
        assert!(
            steps["u1"].config.get("timeout_seconds").is_none(),
            "an omitted --timeout must leave config.timeout_seconds ABSENT, not filled with a \
             collapsed default — got {:?}",
            steps["u1"].config
        );
    ...
    #[test]
    fn stamp_unit_timeout_omitted_flag_leaves_an_explicit_null_null() {
        let mut steps = BTreeMap::new();
        steps.insert("u1".to_string(), crawl_unit_step("u1", serde_json::json!({"timeout_seconds": null})));
        stamp_unit_timeout(&mut steps, None);
        assert!(
            steps["u1"].config.get("timeout_seconds").is_some_and(|v| v.is_null()),
            "an omitted --timeout must not touch an explicit null either — got {:?}",
            steps["u1"].config
        );
    }

    #[test]
    fn stamp_unit_timeout_omitted_flag_never_touches_an_explicit_template_value() {
        // The already-shipped config-file route (#2542/#2586): a
        // hand-authored crawl/review-shaped config can declare its own
        // per-rule `timeout_seconds`. An OMITTED `--timeout` must leave it
        // alone — this was already true before the review round, and stays
        // true (it's the `Some(n)` arm below that changes).
        let mut steps = BTreeMap::new();
        ...
        assert_eq!(
            ...
            Some(45),
            "an omitted --timeout must never touch a hand-authored per-unit timeout_seconds"
        );
    }
    ...
    fn stamp_unit_timeout_omitted_flag_handles_a_null_config() {
        ...
        assert_eq!(
            ...
            "an omitted --timeout must not even allocate a config object — got {:?}",
            steps["u1"].config
        );
    }

    // ── the `Some(n)` arm (the flag was EXPLICIT) — must OVERWRITE, always ──

    #[test]
    fn stamp_unit_timeout_explicit_flag_fills_in_an_absent_value() {
        let mut steps = BTreeMap::new();
        steps.insert("u1".to_string(), crawl_unit_step("u1", serde_json::json!({"plan": "p", "unit": "u1"})));
        stamp_unit_timeout(&mut steps, Some(900));
        assert_eq!(
            steps["u1"].config.get("timeout_seconds").and_then(|v| v.as_u64()),
            Some(900),
            "an absent config.timeout_seconds must be filled from an explicit --timeout; got {:?}",
            steps["u1"].config
        );
    ...
    #[test]
    fn stamp_unit_timeout_explicit_flag_treats_an_explicit_null_as_absent() {
        let mut steps = BTreeMap::new();
        steps.insert("u1".to_string(), crawl_unit_step("u1", serde_json::json!({"timeout_seconds": null})));
        stamp_unit_timeout(&mut steps, Some(900));
    ...
    #[test]
    fn stamp_unit_timeout_explicit_flag_overwrites_an_explicit_template_value() {
        // (#2595 review round 2, MUST FIX 1) The behavior change: an
        // EXPLICIT `--timeout` now wins OUTRIGHT over a hand-authored
        // per-rule `timeout_seconds`, matching `effective_inactivity_
        // timeout_seconds`'s own `Some(n) => ... Cli` rule one layer down
        // — a direct operator override at the point of dispatch, not a
        // fallback that only fills a gap.
        let mut steps = BTreeMap::new();
        steps.insert("u1".to_string(), crawl_unit_step("u1", serde_json::json!({"timeout_seconds": 45})));
        stamp_unit_timeout(&mut steps, Some(900));
        assert_eq!(
            steps["u1"].config.get("timeout_seconds").and_then(|v| v.as_u64()),
            Some(900),
            "an explicit --timeout must overwrite a hand-authored per-unit timeout_seconds outright"
        );
    }

    #[test]
    fn stamp_unit_timeout_explicit_flag_handles_a_null_config() {
        let mut steps = BTreeMap::new();
        steps.insert("u1".to_string(), crawl_unit_step("u1", serde_json::Value::Null));
        stamp_unit_timeout(&mut steps, Some(900));
        assert_eq!(steps["u1"].config.get("timeout_seconds").and_then(|v| v.as_u64()), Some(900));
    ...
    #[test]
    fn stamp_unit_timeout_explicit_flag_never_clobbers_a_non_object_non_null_config() {
        // (#2595 review round 2, CONSIDER 7) `Step.config` is untyped — a
        // string/number/array/bool CAN reach here. Replacing the whole
        // value with `{"timeout_seconds": n}` would silently destroy
        // whatever it held; leaving it untouched instead means the step's
        // own parse fails loudly by name (`requires config.plan`) rather
        // than eating the malformed config quietly.
        for bad in [
            ...
            let mut steps = BTreeMap::new();
            steps.insert("u1".to_string(), crawl_unit_step("u1", bad.clone()));
            stamp_unit_timeout(&mut steps, Some(900));
            assert_eq!(
                steps["u1"].config, bad,
                "a non-object, non-null config must be left completely untouched; got {:?}",
                steps["u1"].config
            );
        }
    }
    ...
    fn stamp_unit_timeout_ignores_every_other_step_kind() {
        ...
        let mut steps = BTreeMap::new();
        for (id, kind) in [("s1", "mission.coder"), ("s2", "dispatch.internal")] {
            steps.insert(
                id.to_string(),
                crew::types::Step {
                    id: id.to_string(),
    ...
    // (#2595 review round 2, MUST FIX 4, vacuity mutation 2) A prior round
    // of `grow_phase_stamps_the_resolved_timeout_onto_a_grown_crawl_unit_
    // step` declared exactly ONE grow template, so a mutant that scoped the
    // `stamp_unit_timeout` call to only the FIRST template's batch
    // (`if out.is_empty() { stamp_unit_timeout(...) }`) left that test
    // green too. This harness grows TWO templates from the SAME producer
    // and asserts the stamp lands on both batches, closing that gap. Both
    // the omitted-flag (`None`, nothing stamped, standing tier survives)
    // and explicit-flag (`Some(n)`, overwrite) arms are exercised through
    // the real `grow_phase` call site, not just the extracted helper above.
    type TwoTemplateCrawlPhaseFixture = (
        mission_config::PhaseConfig,
        BTreeMap<String, Vec<String>>,
        BTreeMap<String, crew::types::Task>,
        BTreeMap<String, crew::types::Step>,
    );

    fn two_template_crawl_phase_fixture() -> TwoTemplateCrawlPhaseFixture {
        let dir = TempDir::new().unwrap();
        // Leaked so the path outlives this fixture fn — the returned
        // `PhaseConfig` only holds the rendered path string, not the
        // `TempDir` guard, and this fixture is only ever used inside a
        // single test body where the process-lifetime leak is harmless.
        let plan_path = Box::leak(Box::new(dir.keep())).join("plan.json");
    ...
    fn grow_phase_explicit_flag_stamps_every_grown_crawl_unit_step_across_two_templates() {
        ...
        assert_eq!(batches.len(), 2, "two grow templates declared, two batches expected");
        for (event, grown_tasks, grown_steps, producer_error) in &batches {
    ...
    #[test]
    fn grow_phase_omitted_flag_leaves_every_grown_crawl_unit_step_untouched() {
        // (#2595 review round 2, MUST FIX 1, red-prove) The `None` arm's
        // full-path pin: an omitted `--timeout` must leave BOTH templates'
        ...
        for (event, grown_tasks, grown_steps, producer_error) in &batches {
            ...
            // (#2595 review round 2, MUST FIX 1b provenance bridge) Feed
            // the grown step straight into the SAME parser the real
            // container-agentic dispatch path uses
            // (`crates/darkmux-lab/src/crawl/unit_step.rs::UnitStepConfig::
            // from_step`) to prove the absence survives all the way to the
            ...
            // resolves to the env/config tier, not `"cli"` — this crate
            // cannot reach that private function directly, so the bridge
            // stops at the shared, public parse boundary.
            let cfg = darkmux_lab::crawl::unit_step::UnitStepConfig::from_step(grown_step)
                .expect("a grown unit's config must still parse");
            assert_eq!(
                cfg.timeout_seconds, None,
                "template `{}`: an omitted --timeout must leave \
                 UnitStepConfig::timeout_seconds (-> DispatchOpts::timeout_override_seconds) at \
                 None, so effective_inactivity_timeout_seconds resolves the standing env/config/ \
                 built-in tier instead of reporting a false `cli` provenance",
                event.task_template
            );
        }
    }
    ...
    // that map's mint-time persist loop) without needing a real dispatch.
    #[test]
    fn a_statically_declared_crawl_unit_step_is_stamped_when_the_flag_is_explicit() {
        let cfg: mission_config::MissionConfig = serde_json::from_value(serde_json::json!({
            "id": "m", "name": "M", "schema_version": "3.2", "inputs": [],
    ...
    #[test]
    fn a_statically_declared_crawl_unit_step_is_untouched_when_the_flag_is_omitted() {
        let cfg: mission_config::MissionConfig = serde_json::from_value(serde_json::json!({
            "id": "m", "name": "M", "schema_version": "3.2", "inputs": [],
            "phases": [{"id": "p", "tasks": [{"id": "t", "role_id": "crawler", "steps": [
                {"id": "s", "kind": "crawl.unit", "config": {"plan": "/p.json", "unit": "u1"}}

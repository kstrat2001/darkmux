...
mod tests {
    ...
    fn expand_fills_all_vars() {
        let out = expand("{ticket}/{phase}", &vars(Some("SAMPLE-4101"))).unwrap();
        assert_eq!(out, "SAMPLE-4101/s1-fix");
        let out = expand("{ticket}: {subject}", &vars(Some("SAMPLE-4101"))).unwrap();
        assert_eq!(out, "SAMPLE-4101: fix the page copy");
    }
    ...
    fn valid_branch_accepts_and_rejects() {
        assert!(valid_branch("SAMPLE-4101/s1-fix"));

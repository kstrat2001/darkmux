...
| `DARKMUX_AUDIT_DIR` | unset → AuditFileSink off | When set, flow records ALSO write to a hash-chained per-day JSONL under this directory (AuditFileSink, #163). **POSIX-only** (Linux/macOS; Windows is unsupported: the env var is recognized but the sink is skipped). Cross-process safe via `flock(2)`. `darkmux flow integrity-check` walks the chain and **exits with status 2 on any chain break** so cron/CI can flag tampering. `darkmux doctor` rolls up the same result. A DETECTION substrate: it makes a later edit to a retained record detectable by `flow integrity-check`. It does not prevent alteration, and running it does not make an operator compliant with any regulatory framework. |

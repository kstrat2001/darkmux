...
- **`detail`** (one sentence, two at the absolute most): state the defect, why it's wrong, and the fix direction — packed into that one or two sentences, not three separate paragraphs. Don't restate the mechanical fix in prose when `suggestion` already carries it verbatim; use the sentence budget to say what `suggestion` can't (the *why*). If a finding depends on code not in the diff, say so in the same sentence and frame it as something the author should verify.
